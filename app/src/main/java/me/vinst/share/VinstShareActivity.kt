...
class VinstShareActivity : AppCompatActivity() {
    ...
    private fun resolveInstalledVinstPackage(pm: PackageManager, url: String): String? {
        VINST_PACKAGE_CANDIDATES.firstOrNull { candidate ->
            pm.getLaunchIntentForPackage(candidate) != null
        }?.let { return it }
        ...
        return pm.queryIntentActivities(sendProbe, PackageManager.MATCH_DEFAULT_ONLY)
            .asSequence()
    ...
    private fun buildDeepLinkIntent(url: String, targetPackage: String, pm: PackageManager): Intent? {
        ...
        val deepLinks = listOf(
            "vinst://share?url=$encoded",
            "vinst://open?url=$encoded",
            "https://vinst.app/share?url=$encoded"
        )
